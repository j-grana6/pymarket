...
class OrderBook(object):
    ...
    def __init__(self, bids, asks, vol=0):
        ...
        bids : dict
            List contains 3-tuples of bids in the form (price, quantity, time)
        ...
        self.truep = 100
        self.price = 100
        self.transactions = {} ##TODO Change to default dict
        self.vol = vol
    ...
    def order(self, agentid, price, size, time, side):
        """ General function to execute order"""
        if price == 'Market':
            self._market_order(side, size, time)
            self.transactions[time[0]].append([agentid, self.price, size, time, side])
        else :
            self._limit_order(agentid, price, size, time, side)


    #@profile
    def _limit_order(self, agent_id, order_price, order_size, time, order_side):
        """
        Adds an order to the existing order book.
        """
        if order_side == 'S':
            try:
                self.asks[order_price].append([order_price, order_size, time, agent_id])
            except KeyError:
                self.asks[order_price] = [[order_price, order_size, time, agent_id]]
        else:
            try:
                self.bids[order_price].append([order_price, order_size, time, agent_id])
            except KeyError:
                self.bids[order_price] = [[order_price, order_size, time, agent_id]]
        ...
        if order_side == 'S':
            while order_size > 0:
                ...
                self.transactions[time[0]].append([highest_bid[3],
                                      highest_bid[0], size, highest_bid[2], 'B'])
                highest_bid[1] = highest_bid[1] - size
                self.price = highest_bid[0]
                if highest_bid[1] == 0:
                    if highest_bid[3] != 'Me':
                        self.Agents[highest_bid[3]].position = ('out', 'NA')
                    _ = self.bids[self.price].pop(0)
                else:
                    if highest_bid[3] != 'Me':
            ...
            while order_size > 0:
                entry = min(self.asks.keys())  # TODO Transaction in different order than order
                lowest_ask = self.asks[entry][0]
                size = min(lowest_ask[1],  order_size)
                self.transactions[time[0]].append([lowest_ask[3],
                                                  lowest_ask[0], size, lowest_ask[2], 'S'])
                lowest_ask[1] = lowest_ask[1] - size
                self.price = lowest_ask[0] # Record the price
                if lowest_ask[1] == 0:
                    if lowest_ask[3] != 'Me':
...
class Trader(object):
    ...
    def query_agent(self):# TODO add documentation
        ...
        if np.random.uniform() <  p_partic:
            ...
            self.Book.order(self.agentid, self.oprice, self.oqty,
                            (self.Book.day, self.Book.second), o_side) #place order
            if self.oprice == 'Market':
                self.position = ('out', 'NA')
            else:
